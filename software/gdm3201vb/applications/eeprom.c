...
void printf_eeprom(uint16_t VirtAddress)
{
	uint16_t data;

	EE_ReadVariable(VirtAddress, &data);
	rt_kprintf("read from eeprom addr[0x%x] = 0x%x\n", VirtAddress, data);
}

FINSH_FUNCTION_EXPORT(printf_eeprom, print variable. e.g: printf_eeprom(0x5555);)
FINSH_FUNCTION_EXPORT(EE_ReadVariable, read variable. e.g: uint16_t EE_ReadVariable(uint16_t VirtAddress, uint16_t* Data))
FINSH_FUNCTION_EXPORT(EE_WriteVariable, write variable. e.g: uint16_t EE_WriteVariable(uint16_t VirtAddress, uint16_t Data))
FINSH_FUNCTION_EXPORT(EE_Init, init emulated eeprom. e.g: void EE_Init(void))
#endif
